...
import {EApproachChoice, EDateMode, EGender, EVerificationStatus} from "../types/user.types";
...
describe('UserService', () => {
    ...
    beforeEach(async () => {
        ...
        blacklistedRegionRepository = module.get<Repository<BlacklistedRegion>>(getRepositoryToken(BlacklistedRegion));
    });
    ...
    describe('createUser', () => {
        it('should create a new user with images and blacklisted regions', async () => {
            ...
            (blacklistedRegionRepository.save as jest.Mock).mockResolvedValue({ id: 1, ...createUserDto.blacklistedRegions[0] });

            const result = await userService.createUser(createUserDto, mockImages as Express.Multer.File[]);
    ...
    describe('updateUser', () => {
        it('should update an existing user', async () => {
            ...
            const result = await userService.updateUser(userId, updateUserDto, mockImages as Express.Multer.File[]);
        ...
        it('should throw an error if user is not found', async () => {
            ...
            await expect(userService.updateUser(userId, updateUserDto)).rejects.toThrow('User not found');
        });
    });
