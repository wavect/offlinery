...
export class PendingUserController {
    ...
    async deleteUser(
        ...
        return await this.pendingUserService.deleteUser(
            missedEQCall.email,
        );
    }


    @Put("admin/of-shutting-down")
    @OnlyAdmin()
    @ApiExcludeEndpoint()
    async offlineryShuttingDown(): Promise<void> {
        return await this.pendingUserService.sendOfflineryShuttingDown();
    }
}
